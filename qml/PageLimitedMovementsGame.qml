...
MyPage {
    id: pageLimitedMovementsGame

    title: qsTr("Limited movements") //qsTr("Game")
    clip: true
    ...
    QtObject {
        id: gameGUI
        property int sizDots: settings.sizDot*app.dp //Math.ceil( pageLimitedMovementsGame.width/6 )
    ...
    Canvas {
        ...
        Repeater {
            ...
            DragPoint {
                ...
                limitMovementsEnabled: true
                limitMovements: gameSettings.currentLimit
                visible: false
                Timer {
    ...
    DialogEndGame3 {
        ...
        gameType: pageLimitedMovementsGame.typeOfThisGame
        dots: gameSettings.currentColumns*gameSettings.currentRows
        onOkClicked: {
            if( gameSettings.currentGameLevel < (limitedMovementsGameLevels.count-1) ) {
                ...
                //console.log("columns:" + limitedMovementsGameLevels.get( (settings.currentLevel-1) ).columns)
                //console.log("rows:" + limitedMovementsGameLevels.get( (settings.currentLevel-1) ).rows)
                gameSettings.currentColumns = limitedMovementsGameLevels.get( (gameSettings.currentGameLevel-1) ).columns
                gameSettings.currentRows = limitedMovementsGameLevels.get( (gameSettings.currentGameLevel-1) ).rows
                gameSettings.currentPOblique = limitedMovementsGameLevels.get( (gameSettings.currentGameLevel-1) ).pobliq
                gameSettings.currentPVertical = limitedMovementsGameLevels.get( (gameSettings.currentGameLevel-1) ).pobver
                gameSettings.currentLimit = limitedMovementsGameLevels.get( (gameSettings.currentGameLevel-1) ).limit
                gameSettings.currrentBestTime = limitedMovementsGameLevels.get( (gameSettings.currentGameLevel-1) ).time
                initGame()
            } else {
                app.currentPage = "PageLimitedMovementsGameLevels.qml"
            }
        }
        onReplayClicked: {
            var cntx = canvas.getContext("2d");
