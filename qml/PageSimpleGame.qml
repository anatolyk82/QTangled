...
MyPage {
    ...
    title: qsTr("Classic game") //qsTr("Game")
    clip: true
    ...
    function checkFinish() {
        ...
        if( !intersectionExists ) {
            ...
            //get the best time of this level before saving a new result
            try {
    ...
    DialogEndGame3 {
        ...
        onOkClicked: {
            if( gameSettings.currentGameLevel < (simpleGameLevels.count-1) ) {
                ...
                //console.log("columns:" + simpleGameLevels.get( (settings.currentLevel-1) ).columns)
                //console.log("rows:" + simpleGameLevels.get( (settings.currentLevel-1) ).rows)
                gameSettings.currentColumns = simpleGameLevels.get( (gameSettings.currentGameLevel-1) ).columns
                gameSettings.currentRows = simpleGameLevels.get( (gameSettings.currentGameLevel-1) ).rows
                gameSettings.currentPOblique = simpleGameLevels.get( (gameSettings.currentGameLevel-1) ).pobliq
                gameSettings.currentPVertical = simpleGameLevels.get( (gameSettings.currentGameLevel-1) ).pobver
                gameSettings.currrentBestTime = simpleGameLevels.get( (gameSettings.currentGameLevel-1) ).time
                initGame()
            } else {
                app.currentPage = "PageSimpleGameLevels.qml"
            }
        }
        onReplayClicked: {
            var cntx = canvas.getContext("2d");
            ...
            SequentialAnimation {
                ScriptAction {
                    script: {
                        ...
                        ///app.contextMenuButtonVisible = !app.contextMenuButtonVisible///new-menu
                    }
                }
